...
const Basket = ({ match: { params: { areaId, itemId}}, foodAreas, order, setFaster, faster, setTime, time, setSelfService, selfService}) => {
  ...
  function GetLink(props) {
    if (props.price != 0) {
      return  <Link to={`/order/${area.id}/${item.id}`} className="Place__order">
                Оплатить {price}
              </Link>;
    }

    return <Link className="Place__order">
              Оплатить {price}
            </Link>;
  }
